...
import type { UrlData, ClickData } from "@/lib/types"
...
export default function AnalyticsPage() {
  ...
  // Fetch URL and clicks data from API
  useEffect(() => {
    const loadData = async () => {
      try {
        setIsLoading(true)
        const response = await fetchUrlData()
  ...
  const filteredData = selectedUrl === "all" ? clicksData : clicksData.filter((item) => item.urlId === selectedUrl)

  // Calculate total clicks
  const totalClicks = filteredData.reduce((sum, item) => sum + item.clicks, 0)

  // Get top performing URLs
  const topUrls = [...urlData].sort((a, b) => b.totalClicks - a.totalClicks).slice(0, 5)
              ...
              <CardHeader>
                ...
                <CardDescription>
                  {selectedUrl === "all"
                    ...
                    : `Clicks for ${urlData.find((url) => url.id === selectedUrl)?.shortUrl}`}
                </CardDescription>
              </CardHeader>
